...
<!-- <xsl:key name="byte-key" match="nodeset($ranges)/a[not(@bactxt) and not(@bbctxt) and not(contains(@u, ' '))]" use="@b"/> -->
...
<!-- <xsl:key name="uni-key" match="nodeset($ranges)/a[not(@uactxt) and not(@ubctxt) and not(contains(@b, ' '))]" use="@u"/> -->
...
<!--    <p>There are <xsl:value-of select="count(nodeset($ranges)//a)"/> range elements.</p> -->
    <h2>Chart</h2>
...
<xsl:template name="cell">
    ...
    <xsl:variable name="match" select="$ranges[position() = 1]/a[@b=$cell_hex]|key('byte-key', $cell_hex)"/>
    ...
    <xsl:choose>
        ...
        <xsl:when test="(not(key('uni-key', $match/@u)) and not($ranges[position() = 1]/a[@u=$match/@u]))
...
<xsl:template match="a" mode="errors">
    <xsl:if test="not(@bactxt) and not(@bbctxt) and not(contains(@u, ' '))">
        <xsl:if test="not(key('uni-key', @u)|$ranges[position() = 1]/a[@u=current()/@u])">
            <p>No reverse mapping for U+<xsl:value-of select="@u"/> mapped from 0x<xsl:value-of select="@b"/></p>
        </xsl:if>
        <xsl:if test="count(key('uni-key', @u)|$ranges[position() = 1]/a[@u=current()/@u]) > 1">
    ...
    <xsl:if test="not(@uactxt) and not(@ubctxt) and not(contains(@b, ' '))">
        <xsl:if test="not(key('byte-key', @b)|$ranges[position() = 1]/a[@b=current()/@b])">
            <p>No reverse mapping for 0x<xsl:value-of select="@b"/> mapped from U+<xsl:value-of select="@u"/></p>
        </xsl:if>
        <xsl:if test="count(key('byte-key', @b)|$ranges[position() = 1]/a[@b=current()/@b]) > 1">
...
<xsl:template name="range-errors">
    ...
    <xsl:if test="not(@bactxt) and not(@bbctxt) and not(contains(@u, ' '))">
        <xsl:if test="not(key('uni-key', @u)|$ranges[position() = 1]/a[@u=$hu])">
            <p>No reverse mapping for U+<xsl:value-of select="@u"/> mapped from 0x<xsl:value-of select="@b"/></p>
        </xsl:if>
        <xsl:if test="count(key('uni-key', @u)|$ranges[position() = 1]/a[@u=$hu]) > 1">
            <p>Multiple mapping from U+<xsl:value-of select="@u"/> to at least 0x<xsl:value-of select="@b"/></p>
        </xsl:if>
    </xsl:if>
    <xsl:if test="not(@uactxt) and not(@ubctxt) and not(contains(@b, ' '))">
        <xsl:if test="not(key('byte-key', @b)|$ranges[position() = 1]/a[@b=$hb])">
            <p>No reverse mapping for 0x<xsl:value-of select="@b"/> mapped from U+<xsl:value-of select="@u"/></p>
        </xsl:if>
        <xsl:if test="count(key('byte-key', @b)|$ranges[position() = 1]/a[@b=$hb]) > 1">
            <p>Multiple mapping from 0x<xsl:value-of select="@b"/> to at least U+<xsl:value-of select="@u"/></p>
        </xsl:if>
    </xsl:if>
