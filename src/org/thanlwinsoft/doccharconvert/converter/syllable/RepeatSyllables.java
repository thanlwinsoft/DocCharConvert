...
public class RepeatSyllables implements SyllableChecker
{
  ...
  */
      ...
      Vector <Syllable> syllables, boolean debug)
  {
    ...
    {
      boolean hasSeparator = !(scripts[newSide].getRepeatChar().equals(""));
    ...
    {
      boolean hasSeparator = !(scripts[oldSide].getRepeatChar().equals(""));
      int offset = 1;
      ...
      for (int i = offset; i< syllables.size(); i++)
      {
        ...
        if (s.isKnown() && prev.isKnown() && s.equals(prev) && 
            ((hasSeparator == false) || 
             testSeparator.equals(separator) || testSeparator.equals(" ")))
        {
          if (hasSeparator)
          {
            if (i - offset > 0)
            {
              String testBefore = syllables.get(i-offset-1).getOriginalString();
              if (testBefore.equals(separator)) continue;
            }
            if (i + 1 < syllables.size())
            {
              String testAfter = syllables.get(i + 1).getOriginalString();
  ...
  public boolean initialize(Script [] scripts, Object [] args)
  {
    ...
    else if (args.length == 3)
    {
      try
      {
        ...
        this.scripts[0] = scripts[0];
        this.scripts[1] = scripts[1];
        scripts[0].setRepeatChar((repeaterSide == 0), args[1].toString());
        scripts[1].setRepeatChar((repeaterSide == 1), args[2].toString());
        return true;
      }
